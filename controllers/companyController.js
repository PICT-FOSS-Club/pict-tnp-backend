  ...
  try {
      ...
      for (const prevQualStudent of previousQualifiedStudents) {
          if (qualStudents.includes(prevQualStudent.studentEmail)) {
              await Company.findOneAndUpdate({ id: companyId, 'appliedStudents.studentEmail': { $eq: prevQualStudent.studentEmail } }, { $inc: { 'appliedStudents.$.roundCleared': 1 } });
              await Student.findOneAndUpdate({ email: prevQualStudent.studentEmail, 'appliedCompanies.companyId': { $eq: companyId } }, { $inc: { 'appliedCompanies.$.roundCleared': 1 } });
              if (company.currentRound === company.totalRounds) {
                  company.result = true;
                  (company.ctc <= 20) ? (
                      await Student.findOneAndUpdate({ email: prevQualStudent.studentEmail }, { $set: { 'isLTE20': true } })
                  ) : (
                      await Student.findOneAndUpdate({ email: prevQualStudent.studentEmail }, { $set: { 'isGTE20': true } })
                  )
                  company.save();
              ...
              await Company.findOneAndUpdate({ id: companyId, 'appliedStudents.studentEmail': { $eq: prevQualStudent.studentEmail } }, { $set: { 'appliedStudents.$.studentResult': false } });
              await Student.findOneAndUpdate({ email: prevQualStudent.studentEmail, 'appliedCompanies.companyId': { $eq: companyId } }, { $set: { 'appliedCompanies.$.result': false } });
          }
      }
